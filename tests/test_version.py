...
import pulsar._version as version_mod


def test_get_version_uses_installed_metadata():
    with patch("pulsar._version.version", return_value="1.2.3"):
        assert version_mod.get_version() == "1.2.3"


def test_get_version_falls_back_to_cargo_toml():
    with (
        patch("pulsar._version.version", side_effect=PackageNotFoundError()),
        patch("pulsar._version._cargo_version", return_value="0.4.0"),
    ):

...
def build_dossier(
    ...
    for col in numeric_cols:
        ...
        all_cluster_means[col] = data.groupby(clusters.values, observed=True)[
            col
        ].mean()
...
def dossier_to_markdown(dossier: TopologicalDossier) -> str:
    ...
    for p in dossier.clusters:
        ...
        md.append(
            "| Feature | Cluster Mean | Global Mean | Rank / {0} | Z-Score | Homogeneity |".format(
                dossier.n_clusters
            )
        )
        md.append("|---|---|---|---|---|---|")
        ...
        for i, row in enumerate(p.central_rows):
            ...
            row_str = ", ".join(
                [
                    ...
                    if k in dossier.global_stats["columns"][:10]
                ]
            )
